...
void EightMandel(const int kPixels[8], int sz) {
  __m256 _x, _y, _x_offset, _y_offset, _xx0, _yy0,
  _zr, _zi, _cr, _ci, _a, _b, _zr2, _zi2, _two, _four, _mask1;
  __m256 _n, _iterations, _mask2;

  _x = _mm256_set_ps(
    kPixels[0] % kWidth, kPixels[1] % kWidth,
    kPixels[2] % kWidth, kPixels[3] % kWidth,
    kPixels[4] % kWidth, kPixels[5] % kWidth,
    kPixels[6] % kWidth, kPixels[7] % kWidth);
  _y = _mm256_set_ps(
    kPixels[0] / kWidth, kPixels[1] / kWidth,
    kPixels[2] / kWidth, kPixels[3] / kWidth,
    kPixels[4] / kWidth, kPixels[5] / kWidth,
  ...
  _xx0 = _mm256_set1_ps(xx0);
  _yy0 = _mm256_set1_ps(yy0);
  _x_offset = _mm256_set1_ps(x_offset);
  _y_offset = _mm256_set1_ps(-y_offset);

  _zr = _mm256_setzero_ps();
  _zi = _mm256_setzero_ps();
  _cr = _mm256_fmadd_ps(_x, _x_offset, _xx0);
  ...
  _zr2 = _mm256_mul_ps(_zr, _zr);
  _zi2 = _mm256_mul_ps(_zi, _zi);
  _a = _mm256_sub_ps(_zr2, _zi2);
  _a = _mm256_add_ps(_a, _cr);
  _b = _mm256_mul_ps(_zr, _zi);
  _b = _mm256_fmadd_ps(_b, _two, _ci);

  _zr = _a;
  _zi = _b;

  _a = _mm256_add_ps(_zr2, _zi2);

  _mask1 = _mm256_cmp_ps(_a, _four, _CMP_LT_OQ);
  _mask2 = _mm256_cmpgt_epi32(_iterations, _n);
  _mask2 = _mm256_and_si256(_mm256_castpd_si256(_mask1), _mask2);
  _mask2 = _mm256_and_si256(_mm256_set1_epi32x(1), _mask2);

  _n = _mm256_add_epi32(_mask2, _n);
...
public:
  ...
  void FlushCalcQueue() {
    ...
    while (!calc_queue.empty()) {
      for (i = 0; i < 8 && !calc_queue.empty(); ++i) {
        points[i] = calc_queue.back();
        calc_queue.pop_back();
      }
      EightMandel(points, i);
    }
  }
#endif
};
